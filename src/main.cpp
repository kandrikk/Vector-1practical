#include "../Include/Vector.h"

int main()
{
    //! Конструктор без параметров
    Vector vec1;
    std::cout << "\nVector v1 created successfully!\n";
    std::cout << "=====================================================================\n";

    //! Конструктор с параметрами
    double arr[] = {2.2, 4.4};
    Vector vec2(arr, 2);
    std::cout << "Конструктор с параметрами \nvec2 = ";
    vec2.printVec();
    std::cout << "=====================================================================\n";

    //PushBack
    vec2.pushBack(5.5);
    std::cout << "PushBack '5.5' \nvec2 = ";
    vec2.printVec();
    std::cout << "=====================================================================\n";

    //PushFront
    vec2.pushFront(1.1);
    std::cout << "PushFront '1.1' \nvec2 = ";
    vec2.printVec();
    std::cout << "=====================================================================\n";

    // Insert 
    //Вставка элемента value в позицию pos
    vec2.insert(3.3, 2);
    std::cout << "Вставка элемента '3.3' в позицию 3 \nvec2 = ";
    vec2.printVec();
    std::cout << "=====================================================================\n";

    // Вставка массива элементов типа ValueType в позицию pos
    double arr2[] = {1.1, 2.2, 5.5};
    Vector vec3(arr2, 3);

    double tmp_arr[] = {3.3, 4.4};
    vec3.insert(tmp_arr, 2, 2);
    std::cout << "Вставка tmp_arr в позицию 3\nvec3 = ";
    vec3.printVec();
    std::cout << "=====================================================================\n";

    // Вставка содержимого вектора в позицию pos
    double arr3[] = {1.1, 2.2, 5.5};
    Vector vec4(arr2, 3);

    double tmp_arr2[] = {3.3, 4.4};
    Vector tmp_vec(tmp_arr,2); // {3.3 , 4.4}
    vec4.insert(tmp_vec, 2);
    std::cout << "Вставка tmp_vec в позицию 3\nvec4 = ";
    vec4.printVec();
    std::cout << "=====================================================================\n";

    // Удаление с конца
    vec4.popBack();
    std::cout << "Удаление с конца\nvec4 = ";
    vec4.printVec();
    std::cout << "=====================================================================\n";

    // Удаление из начала
    vec4.popFront();
    std::cout << "Удаление из начала\nvec4 = ";
    vec4.printVec();
    std::cout << "=====================================================================\n";

    
}